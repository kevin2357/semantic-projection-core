...
def test_mars_square_venus_reasons_between_projected_primitives():
    ...
    row = next(
        ...
        if relationship["attributes"]["canonical_aspect"] == "square"
    )
...
def test_same_source_projects_differently_across_profiles():
    ...
    assert {row["name"] for row in orthodox["objects"]} != {
        row["name"] for row in cognitive["objects"]
    }
...
def test_cognitive_projection_is_deterministic_and_guardrailed():
    ...
    assert all(
        "not_diagnostic" in row["attributes"]["demo_guardrails"]
