...
def fixture_bundle():
    return json.loads((FIXTURES_ROOT / "foundry_temporal_source_bundle_v1_tiny.json").read_text())


def context():
    return ProjectionContext.from_dict(json.loads((EXAMPLES_ROOT / "contexts" / "cognitive_architecture_general_context.json").read_text()))
