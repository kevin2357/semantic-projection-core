...
def test_c6_same_temporal_facts_distinct_profile_semantics():
    ...
    relationships = {
        result["projected_activations"][0]["projected_relationship_type"]
        for result in results.values()
    }
    activator_terms = {
        result["projected_activators"][0]["projected_operator_ref"]
        for result in results.values()
    }
...
def test_c6_summary_retains_semantic_hashes():
    ...
    assert summary["semantic_hashes"]["projected_activations"] == stable_hash(
        full["projected_activations"]
    )
    assert summary["semantic_hashes"]["projected_states"]


def test_c6_upstream_limitations_are_annotated_not_mutated():
    full = _project("cognitive")
