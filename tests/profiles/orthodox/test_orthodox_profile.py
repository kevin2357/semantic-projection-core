...
def test_venus_and_mars_project_to_orthodox_primitives():
    ...
    by_canonical = {row["attributes"]["canonical_object_name"]: row for row in result["objects"]}
...
def test_mars_square_venus_projects_inside_orthodox_ontology():
    ...
    assert {"conflict_drive", "growth_edge", "romance_affection", "values_resources"} <= set(relationship["theme_tags"])
...
def test_demo_merge_uses_source_names_array_without_last_write_loss():
    ...
    assert result["objects"][0]["attributes"]["source_names"] == ["Input A", "Input B"]
