...
def test_woofmapped_contexts_have_distinct_audience_contracts():
...
def test_production_metadata_uses_durable_capability_language():
    ...
    result = project_foundry_temporal_bundle(
        bundle, profile_id="woofmapped_astrology.v0", profile_version="0.1.0", context=context, output_mode="full"
    )
...
def test_woofmapped_context_modes_reuse_same_declared_primitive_scope():
    bundle = json.loads(BUNDLE.read_text(encoding="utf-8"))
    artifacts = []
    for name in ("woofmapped_handler_guidance_context.json", "woofmapped_dog_direct_context.json"):
        result = project_foundry_temporal_bundle(
            ...
            context=ProjectionContext.from_dict(_load(name)),
