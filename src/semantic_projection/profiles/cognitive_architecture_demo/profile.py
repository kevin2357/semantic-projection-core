...
class CognitiveArchitectureDemoProfile:
    """Expanded experimental mapping proof, not a psychological assessment."""
    ...
    def validate_context(self, context: ProjectionContext) -> list[dict[str, Any]]:
        ...
        if context.context_id != CONTEXT_ID:
            warnings.append({
                "code": "cognitive_demo.context.nonstandard",
                "message": "Context accepted, but the demo defines one reference context.",
                "details": {"expected": CONTEXT_ID, "received": context.context_id},
            })
        if context.subject_scope not in {"individual", "natal"}:
            warnings.append({
                "code": "cognitive_demo.scope.experimental",
                "message": "The expanded demo is designed for individual Natal source graphs.",
    ...
    def project_object(
        ...
        if source_selection_status(source_object) == "excluded_by_source_selection_policy":
            return []
        house_number = house_cusp_number(source_object)
        if house_number is not None:
            domain = HOUSE_DOMAINS.get(house_number)
            if domain is None:
                return []
            return [{
                "target_key": f"cognitive:domain:house:{house_number}:{source_object.get('id')}",
                "object_type": "cognitive_domain_primitive",
                "name": domain,
                "operators": ["locate", "contextualize", "constrain_domain"],
                "attributes": {
                    "source_house": house_number,
                    "projected_domain": domain,
                    "source_names": [str(source_object.get("name") or f"House {house_number}")],
                    "demo_guardrails": GUARDRAILS,
                },
                "structural_strength_score": source_object.get("structural_strength_score"),
                "projection_relevance_score": 0.72,
                "mapping_rule_id": f"cognitive_architecture_demo.v0.house.{house_number}.{domain}",
                "mapping_rule_version": "0.2.0",
                "conditions_evaluated": [{"condition": "house_domain_supported", "result": True, "value": house_number}],
                "provenance": {"profile_layer": "cognitive_domain_mapping"},
            }]
        ...
        projected_domain = HOUSE_DOMAINS.get(house) if house else None
        structural = source_object.get("structural_strength_score")
        context_component = context_salience(request.context, list(mapping["domains"]))
        ...
        return [{
            "target_key": f"cognitive:{mapping['target_name']}:{source_object.get('id')}",
            ...
            "attributes": {
                ...
                "cognitive_domains": list(mapping["domains"]),
                "source_sign": sign,
            ...
            "mapping_rule_id": (
                f"cognitive_architecture_demo.v0.object."
                f"{canonical_name.lower().replace(' ', '_')}.{mapping['target_name']}"
            ),
            "mapping_rule_version": "0.2.0",
            "conditions_evaluated": [
                {"condition": "natal_object_supported", "result": True, "value": canonical_name},
                {"condition": "sign_mode_resolved", "result": projected_mode is not None, "value": sign},
                {"condition": "house_domain_resolved", "result": projected_domain is not None, "value": house},
            ],
            "provenance": {
                "profile_layer": "cognitive_object_mapping",
    ...
    def project_relationship(
        ...
        return [{
            ...
            "attributes": {
                ...
                "source_process": source_projected["name"],
                "target_process": target_projected["name"],
                ...
                "source_domain": source_projected.get("attributes", {}).get("projected_domain"),
                "target_domain": target_projected.get("attributes", {}).get("projected_domain"),
                ...
                "projection_first_reasoning": {
                    ...
                    **(
                        {"source_mode": source_projected.get("attributes", {}).get("projected_mode")}
                        if source_projected.get("attributes", {}).get("projected_mode") is not None
                        else {}
                    ),
                    **(
                        {"target_mode": target_projected.get("attributes", {}).get("projected_mode")}
                        if target_projected.get("attributes", {}).get("projected_mode") is not None
                        else {}
                    ),
                },
                "demo_guardrails": GUARDRAILS,
            ...
            "projection_relevance_score": relevance,
            "mapping_rule_id": (
                f"cognitive_architecture_demo.v0.aspect."
                f"{aspect}.{mapping['relationship_type']}"
            ),
            "mapping_rule_version": "0.2.0",
            "conditions_evaluated": [
            ...
            "provenance": {"profile_layer": "cognitive_relationship_mapping"},
        }]
    ...
    def finalize(
        ...
        for obj in graph.objects:
            ...
            if attributes.get("projected_domain"):
                domain_counts[attributes["projected_domain"]] = domain_counts.get(attributes["projected_domain"], 0) + 1
        for relationship in graph.relationships:
            mode = relationship.get("attributes", {}).get("interaction_mode")
            if mode:
                interaction_counts[mode] = interaction_counts.get(mode, 0) + 1
        graph.summary["profile_scope"] = "expanded_natal_operator_mode_domain_interface_and_sdk_aspects"
        graph.summary["experimental_demo"] = True
        graph.summary["validated_psychological_model"] = False
        graph.summary["diagnostic_use_permitted"] = False
        ...
        graph.summary["supported_house_domains"] = {str(k): v for k, v in sorted(HOUSE_DOMAINS.items())}
        ...
        graph.summary["projected_domain_counts"] = dict(sorted(domain_counts.items()))
